...
namespace RealmServer
{
    class RealmServer
    {
        static void Main()
        {
            ...
            Log.Message(LogType.Init, "___________________________________________");
            Log.Message(LogType.Init, "Traduction par Arctium-Fr http://www.arctium-fr.vv.si/");
            Log.Message();

            Log.Message(LogType.Normal, "Demarrage de Arctium RealmServer...");
            ...
            if (RealmClass.realm.Start(RealmConfig.BindIP, (int)RealmConfig.BindPort))
            {
                ...
                Log.Message(LogType.Normal, "RealmServer se connecte {0} port {1}.", RealmConfig.BindIP, RealmConfig.BindPort);
                Log.Message(LogType.Normal, "RealmServer est demarrer!");
                ...
                new Thread(() =>
                {
                    ...
                    while (true)
                    {
                        Log.Message(LogType.Debug, "Mise a jours Realm List...");
                        ...
                        using (var result = DB.Realms.Select("SELECT * FROM realms"))
                        {
                            for (int i = 0; i < result.Count; i++)
                            {
                                ...
                                if (start)
                                {
                                    Log.Message(LogType.Normal, "Ajout de Realm \"{0}\"", Name);
                                    start = false;
                                }
                            }
                        }
                ...
                Log.Message(LogType.Error, "RealmServer n'a pas pu demarrer: ");

            Log.Message(LogType.Normal, "Memoire total: {0} Kilobytes", GC.GetTotalMemory(false) / 1024);
        }
    }
}
